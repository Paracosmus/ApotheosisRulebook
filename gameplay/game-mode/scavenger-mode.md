...
{{ briefing `Scavenge Mode` `É o modo de jogo onde os Heróis exploram os ermos prateados em busca de resgatar tesouros e segredos antigos.` }}
...
O time então se desloca par a borda dos ermos prateados, e uma vez que adentrem os ermos, a viagem começa oficialmente e o time está no _Modo de Vasculhador_.

---

## Contadores de Viagem

Sempre que o time avançar par longe das terras descongelados, adicione um {{ counterTravel }} à viagem, usado para acompanhar o progresso do time. Quando o time voltar em direção às terras descongeladas, remova um {{ counterTravel }} da viagem.

O número de {{ counterTravel }} na viagem representa a distância do time em relação à borda dos ermos prateados, e é usado para determinar a quantidade de cartas de gelo prateado que devem ser adicionadas ao {{ scenario }}.
...
>>> Atrito
  ...
  O {{ dmg }} de atrito é causado pelo contato com o gelo prateado, ou por simplesmente estar dentro da área onde o gelo prateado expulsa o éter. O atrito causa danos físicos e mentais aos Heróis, representando a perda de "_energia vital_" que o gelo prateado causa.

  Nesta etapa cada personagem:
  - Perde {{ n'10' }} de {{ hp }}
  - Consome {{ n'1' }} de **energia**
...
>>> Ação
  ...
  Se decidirem _avançar_ ou _recuar_ vá para o próximo passo, o desafio de sobrevivência.
...
>>> Desafio de Sobrevivência
  ...
  Um dos jogadores rola {{ e1d6 }} (considera a regra de [rolagem perfeita](/gameplay/die#dados-extras-rolagem-perfeita)), para determinar o desafio de sobrevivência conforme a tabela abaixo. O mestre pode adicionar um contexto narrativo e descrever o cenário e os perigos que os Heróis enfrentam.

  #### Atributo do Desafio

  O atributo associado ao desafio de sobrevivência é o atributo que os Heróis devem usar no teste em conjunto para superar o desafio, sendo que os testes não estão relacionados a nenhum {{ knowledge }}.
  ...
  6-3  | _Sem Teste_ | Planície               | Travessia fácil e segura pela neve prateada.
  ...
  >
  > * Cada personagem recupera ( {{ p'10' }} ✱ Número de rolagens perfeitas ) de {{ hp }}
  > * Cada personagem recupera ( {{ p'2' }} ✱ Número de rolagens perfeitas ) de {{ sp }} e {{ mp }}

  #### Dificuldade

  > _Dificuldade_ = Número de Personagens do Time ✱ ( Número de Cartas de Gelo Prateado no {{ scenario }} + Dificuldade Base do Desafio + Modificadores Narrativos )
  ...
  ##### Número de Cartas de Gelo Prateado no {{ scenario }}
  ...
  Isto é representado em jogo adicionando ao {{ scenario }} cartas que tenham pelo menos as propriedades **TERRENO** e **GELO PRATEADO** simultaneamente. Estas cartas devem vir da pilha de descarte primariamente, se não houver mais cartas com estas propriedades na pilha de descarte, o mestre busca as cartas no {{ sanctuary }}, se também não houver o suficiente, o mestre pode usar da sua mão.

  A cada **5** {{ counterTravel }}, adicione uma destas cartas ao {{ scenario }}.
  ...
  !!!secondary Exemplo - Redução de Dificuldade
   ...
   Cartas como a {{ house }} _Vatel_ e a {{ class }} _Vasculhador_ são exemplos de cartas relacionadas aos vasculhadores. Bem como animais que navegam e sobrevivem bem nos ermos prateados, como _Cisnes_ e criaturas mágicas de gelo prateado.
  !!!

  !!!secondary Exemplo - Aumento de Dificuldade
